...
D3DApp::~D3DApp()
{
	// 我们需要等到GPU处理完队列中的命令后，才能销毁GPU仍在引用的任何资源。
	// 否则当应用程序退出时GPU可能会崩溃。
...
bool D3DApp::InitDirect3D() 
{
	...
	HRESULT hardwareResult = D3D12CreateDevice(
		...
		D3D_FEATURE_LEVEL_11_0, // minimun feature level
	...
	ThrowIfFailed(md3dDevice->CheckFeatureSupport(
		D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS,	// 检测的feature类型，这里检测mass
		&msQualityLevels,
		sizeof(msQualityLevels)));

	// 得到支持最大的quality level
...
void D3DApp::CreateCommandObjects()
{
	...
	queueDesc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT;	// 存储一个由GPU直接执行的命令列表
	...
	ThrowIfFailed(md3dDevice->CreateCommandList(
		0,							// node mask(0表示单个GPU系统)
		D3D12_COMMAND_LIST_TYPE_DIRECT,
		mDirectCmdListAlloc.Get(),	// Associated command allocator
		nullptr,					// Initial PipelineStateObject(初始化目的，不包含任何draw命令)
